...
function readInit(message, tokens) {
	baseProtocol.readInitParameters(message, tokens, true);
	if (message.ariVersion == "1.9.0") {
		...
		// hence, for a Data Adapter we prefer to override and refuse,
		// because, otherwise, the connection would fail anyway;
		// on the other hand, this does not affect Metadata Adapters
		message.initResponseParams = null;
		message.diagnostics = "Unsupported Proxy Adapter version";
	}
}
