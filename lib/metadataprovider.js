...
var proto = require('./metadataprotocol').metadata;
var MetadataReader = require('./metadataprotocol').MetadataReader;
 ...
 */
function MetadataProvider(stream, params, credentials, keepaliveInterval) {
	...
	// close message possibly expected
	var closePossible = true;
	...
	var credentialParams = baseProto.getCredentials(credentials, closePossible);
	if (credentialParams != null) {
		stream.write(proto.writeRemoteCredentials(credentialParams));
	}
	...
	function startKeepalives(keepaliveHint) {
		...
		baseProto.handleKeepalives(keepaliveInterval, keepaliveHint, function() {
			stream.write(proto.writeKeepalive());
			return ok;
		});
		stream.on('close', function() { ok = false; });
	}
	...
	function handleIncomingMessage(data) {
		var message;
		reader.parse(data, initPending, closePossible);
		while (!reader.isEmpty()) {
			...
			if (message.verb === "init") {
				...
				if (message.initResponseParams != null) {
					if (message.initResponseParams[specialParameters.ARI_VERSION] === "1.8.2") {
						// downgrade
				...
				if (closeReason != null) {
					stream.destroy(new Error("Close requested by the counterpart with reason: " + closeReason));
				} else {
					stream.destroy(new Error("Close requested by the counterpart"));
				}
				return;
			}
