 ...
 * @param {Object} [credentials] optional credentials to be submitted to the remote LS proxy adapter.
 * The credentials are needed only if the Proxy Adapter is configured to require Remote Adapter authentication.
 * If needed, the supplied object should contain a "user" and a "password" field; otherwise, null can be specified.
 */
function DataProvider(reqRespStream, notifyStream, isSnapshotAvailable, credentials) {
	...
	handlers = {
		...
		'init' : function(message) {
			if (message.initResponseParams != null) {
				// since 1.8.1
				proto.addCredentials(message.initResponseParams, credentials);
			}
