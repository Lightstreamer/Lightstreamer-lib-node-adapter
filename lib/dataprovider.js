 ...
 */
function DataProvider(stream, isSnapshotAvailable, credentials, keepaliveInterval) {
	...
	// Heuristic check for backward compatibility mistakes
	if (isSnapshotAvailable && (isSnapshotAvailable instanceof Stream)) {
		throw new Error("Two streams received for DataProvider; missing alignment after upgrade?"); 
	}
		...
		while (!reader.isEmpty()) {
			...
			if (message.verb === "init") {
				...
				if (message.initResponseParams == null) {
					...
					reply(proto.writeInitException(message.id, message.diagnostics, "data"));
					return;
				}
