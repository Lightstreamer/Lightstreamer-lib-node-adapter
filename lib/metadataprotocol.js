...
exports.metadata = {
	...
	writeInitException : function(requestId, exceptionMessage, exceptionType) {
		return writeSimpleExceptionOld(requestId, metadataMethods.METADATA_INIT, exceptionMessage, exceptionType);
	},
	/**
...
/**
 * Encodes an unsuccessful reply.
 * Messages are encoded as strings in the backward-compatible way according to ARI protocol 1.9.0 or later.
 *
 * @param {String} requestId the originating request id
 * @param {String} requestType the originating request method name
 * @param {String} exceptionMessage the exception message
 ...
 * @private
 */
function writeSimpleExceptionOld(requestId, requestType, exceptionMessage, exceptionType) {
	return protocol.implodeMessage(requestId, requestType,
		protocol.encodeMetadataException(exceptionType),
		protocol.encodeStringOld(exceptionMessage));
}
...
function readInit(message, tokens) {
	return baseProtocol.readInitParameters(message, tokens, false);
}
...
function readTableInfo(tokens, skipSelector) {
	...
	tableInfo.dataAdapter = protocol.decodeString(tokens[7]);
	tableInfo.schemaName = protocol.decodeString(tokens[9]);
	tableInfo.firstItemIndex = protocol.decodeInteger(tokens[11]);
	tableInfo.lastItemIndex = protocol.decodeInteger(tokens[13]);
	if (!skipSelector)
		tableInfo.selector = protocol.decodeString(tokens[15]);
	return tableInfo;
}
