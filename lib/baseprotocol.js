...
function getSupportedVersion(protocolVersion, isData) {
	...
	if (protocolVersion != null) {
		if (protocolVersion.startsWith("1.8.")) {
			...
			// hence, for a Data Adapter we prefer not to accept, because, otherwise,
			// the connection would fail anyway; on the other hand, this does not affect
			// Metadata Adapters, so in this case we can accept
			if (isData) {
				return null;
			} else {
				return "1.9.1";
				// the upgrade should be taken care by the caller
			}
		} else {
			return "1.9.1"; // maximum version supported
			// any downgrade should be taken care by the caller
		}
	} else {
...
function readInitParameters(message, tokens, isData) {
	var i;
	var ariVersion = null;
	...
	var advertisedVersion = getSupportedVersion(ariVersion, isData);
